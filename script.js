...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchPanelData() {
    fetch(panelDataUrl)
      ...
      .then((csvText) => {
        ...
        rows.forEach((row) => {
          ...
          if (cols.length > 5) {
            ...
            const energyProduced = parseFloat(cols[5]); // DAILY_YIELD'in altıncı sütunda olduğunu varsayarak
            console.log(
              `Source Key: ${sourceKey}, Energy Produced: ${energyProduced}`,
            ); // Kaynak anahtarı ve üretilen enerjiyi
            const panelIndex = getPanelIndex(sourceKey); // Kaynak anahtarını panel indeksine eşlemek için özel fonksiyon
            if (panelIndex >= 0 && panelIndex < 6 && !isNaN(energyProduced)) {
              panelData[panelIndex].push(energyProduced);
            }
      ...
      .catch((error) => {
        console.error("Error fetching panel data:", error);
        document.getElementById("panel-data-list").innerHTML =
          '<li class="list-group-item text-danger">Error loading panel data</li>';
      });
  }

  // Custom function to map source key to panel index
  function getPanelIndex(sourceKey) {
    const panelMapping = {
      "1BY6WEcLGh8j5v7": 0,
      "1IF53ai7Xc0U56Y": 1,
